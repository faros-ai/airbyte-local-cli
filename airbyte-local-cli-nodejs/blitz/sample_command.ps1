$env:FAROS_API_KEY = ""
$env:FAROS_API_URL = "https://dev.api.faros.ai"
 
jq --arg api_key $env:FAROS_API_KEY '.src.config.api_key = $api_key | .dst.config.edition_configs.api_key = $api_key' ./resources/graph_copy.json.template > ./resources/graph_copy.json
jq --arg api_key $env:FAROS_API_KEY '.src.config.api_key = $api_key | .dst.config.edition_configs.api_key = $api_key' ./resources/graph_copy_docker_options.json.template > ./resources/graph_copy_docker_options.json
jq --arg api_key $env:FAROS_API_KEY '.src.config.api_key = $api_key | .dst.config.edition_configs.api_key = $api_key' ./resources/graph_copy_with_tags.json.template > ./resources/graph_copy_with_tags.json
jq --arg api_key $env:FAROS_API_KEY '.src.config.api_key = $api_key' ./resources/graph_copy_src_only.json.template > ./resources/graph_copy_src_only.json
jq --arg api_key $env:FAROS_API_KEY '.dst.config.edition_configs.api_key = $api_key' ./resources/graph_copy_dst_only.json.template > ./resources/graph_copy_dst_only.json
...
./airbyte-local `
  --config-file './resources/graph_copy_src_only.json' `
  --connection-name 'jennie_connection' `
  --src-output-file 'graph_copy_output'
 
# with json string config
./airbyte-local  `
  --src 'farosai/airbyte-faros-graphql-source' `
  --src.api_url $env:FAROS_API_URL `
  --src.api_key $env:FAROS_API_KEY `
  --src.graph 'faros' `
  --src.graphql_api 'v2' `
  ...
  --dst.edition_configs.api_url $env:FAROS_API_URL `
  --dst.edition_configs.api_key $env:FAROS_API_KEY
...
./airbyte-local  `
  ...
  --src.authentication.personal_access_token $env:GITHUB_TOKEN `
...
./airbyte-local `
  ...
  --src.username $env:JIRA_USER_EMAIL} `
  --src.password $env:JIRA_TOKEN `
  --dst farosai/airbyte-faros-destination `
  --dst.edition_configs.api_key $env:FAROS_API_KEY `
